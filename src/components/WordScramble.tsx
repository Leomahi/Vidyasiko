...
const words: ScrambleWord[] = [
  { word: "PHOTOSYNTHESIS", hint: "How plants make food using sunlight", subject: "science" },
  { word: "ALGORITHM", hint: "Step-by-step problem solving", subject: "cs" },
  { word: "TRIANGLE", hint: "A three-sided polygon", subject: "math" },
  { word: "METAPHOR", hint: "A figure of speech (not using like/as)", subject: "english" },
  { word: "ELECTRON", hint: "Negatively charged particle", subject: "science" },
  { word: "FRACTION", hint: "A part of a whole number", subject: "math" },
  { word: "PRONOUN", hint: "A word that replaces a noun", subject: "english" },
  { word: "BOOLEAN", hint: "True or false data type", subject: "cs" },
];
...
export default function WordScramble({ language, onBack, onComplete }: Props) {
  ...
  const scrambled = useMemo(() => words.map((w) => scramble(w.word)), []);
    ...
    <div className="max-w-2xl mx-auto">
        ...
        <div className="flex justify-center gap-2 flex-wrap mb-4">
          {scrambled[current].split("").map((letter, i) => (
            <motion.span
              ...
              className="w-10 h-12 rounded-xl bg-card border-2 border-border flex items-center justify-center text-lg font-bold shadow-sm"
            >
              {letter}
            </motion.span>
          ))}
      ...
      <form onSubmit={handleSubmit} className="flex gap-3 max-w-md mx-auto mb-6">
        <Input
          ...
          className="rounded-xl text-center uppercase tracking-widest font-bold"
          disabled={answered}
          autoFocus
        />
        {!answered && (
          <Button type="submit" className="rounded-xl px-6">Check</Button>
        )}
      </form>
      ...
      {answered && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
          ...
          <Button onClick={handleNext} size="lg" className="rounded-full px-8 mt-4">
            {current < words.length - 1 ? "Next Word" : "Finish 🎉"}
          </Button>
        </motion.div>
      )}
    </div>
