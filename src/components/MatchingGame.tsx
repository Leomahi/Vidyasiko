...
import { Language, MatchPair } from "@/lib/data";
...
export default function MatchingGame({ language, onBack, onComplete, pairs }: Props) {
  ...
  const handleTileClick = (tile: Tile) => {
    ...
    if (selected.pairId === tile.pairId && selected.type !== tile.type) {
      ...
      if (matched.size + 1 === pairs.length) {
        setFinished(true);
        onComplete(Math.max(30, 60 - seconds));
      }
    } else {
  ...
  if (!pairs.length) return <div className="text-center p-8">No matching pairs available.</div>;
  ...
  if (finished) {
    ...
    return (
      <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <div className="flex items-center gap-2 text-xp text-2xl font-bold mb-6"><Zap className="w-7 h-7" /> +{xp} XP</div>
    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-1 text-sm text-muted-foreground"><Clock className="w-4 h-4" /> {seconds}s</div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
        {tiles.map((tile) => {
          ...
          return (
            <motion.button key={tile.id} whileHover={!isMatched ? { scale: 1.05 } : {}} whileTap={!isMatched ? { scale: 0.95 } : {}} onClick={() => handleTileClick(tile)} disabled={isMatched} className={`p-3 rounded-2xl border-2 text-center text-sm font-medium min-h-[80px] flex items-center justify-center transition-colors ${bg}`}>
              {tile.text}
            </motion.button>
          );
        })}
